...
- **Comprehensive gear catalog** — pulls every EoF-era *gear* item (all classes, every itemlevel/tier; non-gear excluded) from Census and persists faithful, *untranslated* CSVs by category (weapons / armor / jewelry-charms), plus a cross-class **Max Health** list for tanks.
...
The catalog is **committed in [`data/`](data/)** — clone or pull the repo to use it directly, no run required. Current snapshot (3,497 EoF-era gear items, all classes, every itemlevel/tier):
...
| `data/maxlife.csv` | 119 | every EoF gear item with Max Health, any class (tank reference) |

Stats are exactly as Census reports them — **no translations**. Quality is driven by `tier` (a column) more than itemlevel — non-gear and low-quality items aren't floored out, since the BiS model (Plan 2) does the real ranking.
