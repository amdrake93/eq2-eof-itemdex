...
**Non-stacking effect buffs (model sums; game takes max).** Items grant stats via two paths: the `modifiers` block (stacks normally) and `effect_list` "When Equipped: Increases \<stat\>" grants (source `effect`, folded since plan 2v). Some effect grants are **named, non-stacking** buffs — e.g. the **"Haste"** effect is *max-wins*: equipping two items that each grant it (Cloak of Flames +25 and a +25/+35 glove) yields only the highest value, not the sum. The model (`StatBlock.Add` / `AddModifiers`) sums all sources additively, so it **double-counts** non-stacking effects — overstating both a set's stats and any upgrade that adds a second copy. Observed 2026-06-24: the gear-import upgrade report (§7) recommended +25-haste-effect gauntlets that don't actually stack with the equipped cloak's haste. Affects the **main BiS optimizer too**, not just import. Later expansions add more effects with **mixed** stacking rules (some stack, some don't), so a real fix needs per-effect stacking-group data (Census exposure unknown) plus a max-vs-sum rule keyed on effect name. Deferred by user 2026-06-24 (managed manually for now). 20 catalog items currently carry an effect-haste grant. Route through brainstorm when scoped.
