...
- **Metric.** Each row's upgrade ΔDPS is the gain of swapping that one worn item (or filling its `Empty`) for the candidate while every other equipped item — **including the slot's other instance** — is held fixed: `Δ = DPS(swapped set) − Set.DPS()`. Positives only; per row keep the best candidate plus one alternative (2nd-best). The same candidate may legitimately top both rows of a two-capacity slot — that is left as-is (the report is advisory; the player buys one). A **weapon** slot's candidates exclude the item worn in the other weapon slot (the player owns one of each physical weapon), so the report never proposes the worn main-hand as an off-hand "upgrade" or vice versa — the same no-duplicate rule the optimizer enforces.
...
**`cmd/weights` pins its baseline main-hand to Soulfire.** The `weights` diagnostic (`cmd/weights/main.go`) selects its baseline main-hand via a `WHERE name LIKE 'Soulfire%'` query, independent of the optimizable Primary slot (§7). It is self-contained (calls `model.TotalDPSDual` directly, never touches `bis.Set`), so it is correct-but-inconsistent — the weight baseline is pinned to Soulfire while the optimizer treats the main-hand as a normal slot. Next step: derive the diagnostic's baseline weapons from the class weapon config / candidate pools, or accept it as a deliberately fixed-baseline diagnostic.
...
**`effect-audit` skip list not yet reviewed.** The effect-list ingestion wrote `data/effect-audit.csv`/`.md` — 191 skipped effect lines (plus 147 captured procs, 25 scored stats) across all items. The 191 skips have not been eyeballed for DPS-relevant wording wrongly dropped (a stat grant whose Census key isn't in `modifierToField`) or procs worth scoring. Next step: review the skip list and promote any real stat/proc into the ingestion rules.
...
**Single-capacity slots omit when there is no upgrade — including the weapons.** The upgrade report emits a per-instance row for a single-capacity slot only when a positive upgrade exists in the bucket; when the worn item is already best (e.g. the main-hand when Soulfire Sabre wins its pool) the slot is simply absent — consistent with Head/Chest, but it can read as "the main-hand is missing." Multi-capacity slots (rings/ears/wrists) already always show every worn instance with a `—` when there is no upgrade (§7). Decision to make: optionally always-show the worn single-capacity weapon slots (Primary/Secondary) with `—` so the equipped weapons are always visible. Route through brainstorm if picked up.
...
**Simulation engine → API → web app.** The long-term direction is to lift the CLI/model core (`internal/model`, `internal/bis`, catalog, census) into a reusable simulation/calc engine, expose it behind an API, and build a web app for browsing all upgrades and building/comparing gear sets — the current model becomes the backend. This is a major decomposition (engine extraction, hosting, data serving, frontend) and gets its own brainstorm → spec → plan(s); not started.
