...
- **Optimizable** — armor, jewelry, cloak, waist, and the `Primary`/`Secondary` weapons. Pre-filled with the equipped item and eligible for re-optimization against the catalog candidate pool. Imported `Primary`/`Secondary` populate the `model.Weapon` structs and **override** the otherwise-fixed Soulfire main-hand (§6) so the sim runs the real weapons.
