...
### Multi-component combat arts (DoTs + detonates) — formula settled, ability-mod split CALIBRATION PENDING (2026-06-15)

Some combat arts deal damage through **more than one component**: an initial hit, a damage-over-time (DoT) tick repeated over a duration, and/or a **termination detonate** (a nuke that fires when the DoT expires). The worked example is **Gushing Wound** (a bleed): a front-load hit, a tick every 4s over a 24s duration, and a detonate on termination. These do not need a new damage model — each component is the **existing direct-CA equation**, summed:

```
per-application = Σ over components [ base_i × (1 + (potency + potencyBonus + artPotencyAdd)/100)
                                            × (1 + mainStatCurve(mainstat)/100)
                                      + abmod_i ] × critFactor
```

Everything already calibrated is reused unchanged (potency pool, `mainStatCurve`, crit). Per-component `base_i` comes from census `effect_list` like every other art — the pull currently keeps only one damage line, so it is **extended to retain all components** of a multi-component art (front-load, periodic tick + interval + duration, termination/detonate). Census being down does not block calibration: Gushing Wound's bases are recoverable from a naked tooltip read.

- **The one new term is `abmod_i` — how ability-mod distributes across components** (the single calibration target). For a direct CA, ability-mod adds in full to the one hit; for a multi-component art it is **split**. Working hypothesis (from prior playthroughs, NOT yet measured): roughly **half the ability-mod breaks off onto the initial hit as its own damage line** (Gushing Wound's `240–400` piercing line is the suspected ability-mod line), and the DoT ticks may carry a share. This is calibrated, not assumed — if the readings disagree, the readings win. Once measured it is written as a **general rule for all DoT/multi-component CAs**, the same way "ability-mod adds in full" generalized for direct CAs.
- **Calibration protocol** (a few Gushing Wound tooltip reads, same spirit as the CA-equation calibration — no permanent fit pipeline): **naked** (ability-mod ≈ 0 → ability-mod-derived lines collapse, exposing which lines are base vs ability-mod and recovering bases by de-scaling with the known `potency_bonus` + naked AGI) → **one ability-mod swap** (a secondary stat, AGI/potency held fixed since AGI+potency+crit are welded together on every item) → **current-gear** (integrated check). AGI and potency cannot be isolated from each other by gear swaps, but they don't need to be — both curves are already globally calibrated; per component we only confirm the standard potency×AGI scaling applies and measure the ability-mod fraction (which *is* isolable, since ability-mod is a freely-varying secondary).

**Rotation behavior (Gushing Wound, settled).** The per-application sum is delivered as a **lump** every `max(effRecast, duration)`; the detonate always lands; **the art is never clipped** (recast early). For Gushing Wound the hold-vs-clip question is doubly trivial: (1) duration 24s ≪ the ~65.6s spam-beats-detonate break-even, so holding to termination always wins the personal-DPS math; and (2) all of the Assassin's single-target bleeds apply a **raid-wide damage-taken debuff** (+0.2% from everyone), so uptime is mandatory by construction regardless of personal DPS. The raid-wide debuff value is **structurally out of scope** for this personal-DPS tool (no model of the rest of the raid) — see §12 deferred items.
...
- **DoT/multi-component CA ability-mod split — CALIBRATION PENDING** (formula settled, §3.1 "Multi-component combat arts"). The per-component damage formula and Gushing Wound's rotation behavior are locked; the one unmeasured term is how ability-mod distributes across components. Working hypothesis (~50% onto the initial hit as its own line, ticks may share) is unverified — needs the naked / ability-mod-swap / current-gear Gushing Wound reads to pin the rule. Census `effect_list` parser also needs extending to retain all damage components (currently keeps one line); not blocking — bases recoverable from the naked read.
